...
import { backendFetch, backendWebSocketUrl } from "./config"
import {
  initializeMusicKit,
...
const selfIdStorageKey = "ateruta-self-id-by-room"
...
export default function App() {
  ...
  let pendingSelfResolution: "host" | "last-active" | null = null
  let lastRound = 0
  ...
  const handleSocketMessage = (raw: string) => {
    ...
    if (event === "room:settings") {
      ...
      const currentSelfId = resolveSelfId(nextSettings)
      setSettings(nextSettings)
      setJoinPending(false)
      setJoined(true)
      if (nextSettings.hostPlayerId !== currentSelfId) {
        window.MusicKit = undefined
        setAuthorized(false)
      }
      if (
        nextSettings.hostPlayerId === currentSelfId &&
  ...
  const handleReveal = (payload: unknown) => {
    ...
    setOwnScore(null)
    if (isHost()) {
      void playSong(nextReveal.song.id, { loop: true }).catch(() =>
  ...
  const prepareSelfResolution = (
    code: string,
    mode: "host" | "last-active"
  ) => {
    const stored = readStoredSelfId(code)
    if (stored !== "") {
      setSelfId(stored)
    }
    pendingSelfResolution = mode
  }

  const resolveSelfId = (nextSettings: RoomSettings) => {
    const code = roomCode()
    const stored = code === null ? "" : readStoredSelfId(code)
    if (stored !== "" && playerById(nextSettings, stored) !== null) {
      setSelfId(stored)
      pendingSelfResolution = null
      return stored
    }
    ...
    if (current !== "" && playerById(nextSettings, current) !== null) {
      if (code !== null) {
        writeStoredSelfId(code, current)
      }
      pendingSelfResolution = null
      return current
    }
    ...
    let resolved = ""
    if (pendingSelfResolution === "host") {
      resolved = nextSettings.hostPlayerId
    } else if (pendingSelfResolution === "last-active") {
      const active = nextSettings.activePlayers
      resolved = active[active.length - 1]?.id ?? ""
    } else if (nextSettings.activePlayers.length === 1) {
      resolved = nextSettings.activePlayers[0]?.id ?? ""
    }

    if (resolved !== "") {
      setSelfId(resolved)
      if (code !== null) {
        writeStoredSelfId(code, resolved)
      }
      pendingSelfResolution = null
    } else {
      setSelfId("")
  ...
  createEffect(() => {
    ...
    if (next !== null && sessionReady()) {
      if (createdRoomCode === next) {
        ...
        prepareSelfResolution(next, "host")
        setRoomChecked(true)
        sendJoin()
        return
      }
      prepareSelfResolution(next, "last-active")
      void checkRoom(next)
    }
  })
  ...
  const initializeApp = async () => {
    try {
      const response = await backendFetch("/api/session")
      if (!response.ok) {
  ...
  const checkRoom = async (code: string) => {
    ...
    setJoinPending(false)
    try {
      const response = await backendFetch(`/api/room/${code}`)
      const body = await response.json().catch(() => null)
      if (!response.ok || !booleanProp(body, "exists")) {
      ...
      return (
        <RoomScreen
          ...
          onPlay={async () => {
            ...
            setPlaybackStatus("loading")
            try {
              await playSong(songId, { duration: currentDuration() })
              setPlaying(true)
...
function HomeScreen(props: {
  ...
  const checkRoom = async (roomCode: string) => {
    setChecking(true)
    try {
      const response = await backendFetch(`/api/room/${roomCode}`)
      if (code() !== roomCode) {
        return
      }
      setChecking(false)
      if (response.ok) {
        const body = await response.json().catch(() => null)
        if (booleanProp(body, "exists")) {
          props.onNavigate(`/room/${roomCode}`)
          return
        }
  ...
  const createRoom = async () => {
    setCreating(true)
    try {
      const response = await backendFetch("/api/room", { method: "POST" })
            ...
            {(player) => (
              <li class="flex items-center justify-between rounded-md bg-slate-50 px-3 py-2">
                <span>
                  ...
                  <Show when={player.id === props.selfId}>
                    <span> (you)</span>
                  </Show>
                </span>
...
function PlaylistPanel(props: RoomScreenProps) {
  ...
  createEffect(() => {
    ...
    loadLibraryPlaylists()
      .then((playlists) => setLibraryItems(playlists))
      .catch(() => props.addToast("Library playlists failed"))
      .finally(() => {
        setLoadingLibrary(false)
      })
  })
  ...
  const filteredLibrary = createMemo(() => {
    ...
    return libraryItems().filter((playlist) =>
...
function PlaylistResults(props: {
  ...
  return (
    <section class="mt-4">
      ...
      <ol aria-label="Playlists" class="space-y-2" role="listbox">
        <For each={items()}>
          {(playlist) => (
            <li
...
function HostSettings(props: RoomScreenProps) {
  ...
  createEffect(() => {
    ...
    if (props.settings.songs.length > 0) {
      setRounds(
        String(props.settings.totalRounds || props.settings.songs.length)
      )
    }
  })
      ...
      <Show when={props.settings.songs.length > 0}>
        <label class="block">
          ...
          <input
            ...
            value={rounds()}
            onInput={(event) => {
              updateRounds(event.currentTarget.value)
            }}
            onChange={(event) => {
              updateRounds(event.currentTarget.value)
            }}
      ...
      <Show
        ...
        fallback={
          <section class="mt-5">
            ...
            <ol aria-label="Suggestions" class="mt-3 space-y-2" role="listbox">
      ...
      <For each={props.players}>
        {(player) => (
          ...
          >
            ...
            <span class="font-black">{player.score} points</span>
          </li>
        )}
      </For>
    </ol>
...
function readStoredSelfId(code: string): string {
  try {
    const raw = window.sessionStorage.getItem(selfIdStorageKey)
    const value = raw === null ? null : JSON.parse(raw)
    return stringProp(recordValue(value), code)
  } catch {
    return ""
  }
}

function writeStoredSelfId(code: string, id: string): void {
  try {
    const raw = window.sessionStorage.getItem(selfIdStorageKey)
    const value = recordValue(raw === null ? null : JSON.parse(raw))
    window.sessionStorage.setItem(
      selfIdStorageKey,
      JSON.stringify({ ...value, [code]: id })
    )
  } catch {
    // Storage is only an optimization for reload/reconnect identity hints.
  }
}
