Feature: Game screen - answering
  ...
  Scenario: Wrong answer feedback shown immediately
    ...
    Then the wrong answer title is displayed as "Wrong: {title}"
  ...
  Scenario: Lockout state disables input
    ...
    And a panel shows "Locked out for {N}s" updating every 100ms
  ...
  Scenario: Own score hides search UI
    ...
    Then a panel shows "You scored 4 points!"
