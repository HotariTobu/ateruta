...
def duration_displayed(spa_page):
    expect(spa_page.get_by_text(re.compile(r"^Duration: .+s$"))).to_be_visible(
        timeout=5000
    )
...
def song_played_locally(spa_page):
    expect(only_status(spa_page, "Playing...")).to_be_visible(timeout=5000)
...
def button_disabled(spa_page, button_name):
    expect(only_button(spa_page, button_name)).to_be_disabled(timeout=5000)
...
def song_loading(spa_page, mock_ws_server, frontend_url):
    ...
    # request is in flight. Register after configure_musickit_api_mock so this override
...
def button_enabled(spa_page, button_name):
    expect(only_button(spa_page, button_name)).to_be_enabled(timeout=5000)
...
# '"Playing..." is displayed' is handled by conftest
...
def non_host_playing(spa_page, mock_ws_server, frontend_url):
    ...
    expect(only_status(spa_page, "Playing...")).to_be_visible(timeout=3000)
...
def reveal_panel_displayed(spa_page):
    panel = only_region(spa_page, "Reveal")
...
def sorted_by_score(spa_page):
    ...
    expect(entries.nth(0).get_by_text("8 points", exact=True)).to_be_visible(
        timeout=5000
    )
    expect(entries.nth(1).get_by_text("8 points", exact=True)).to_be_visible()
    expect(entries.nth(0)).not_to_contain_text("Host", timeout=5000)
    expect(entries.nth(2)).to_contain_text("Host")
    expect(entries.nth(2).get_by_text("Host", exact=True)).to_be_visible()
    expect(entries.nth(2).get_by_text("4 points", exact=True)).to_be_visible()
...
def tied_same_rank(spa_page):
    scoreboard = spa_page.get_by_role("region", name="Scoreboard")
    entries = scoreboard.get_by_role("listitem")
    expect(scoreboard.get_by_text("#1", exact=True)).to_have_count(2)
    expect(entries.nth(0)).to_have_attribute("aria-posinset", "1")
    expect(entries.nth(1)).to_have_attribute("aria-posinset", "1")
...
def you_marker(spa_page):
    expect(
        only_region(spa_page, "Scoreboard").get_by_text("(you)", exact=True)
...
def handicap_badge_scoreboard(spa_page, seconds):
    scoreboard = spa_page.get_by_role("region", name="Scoreboard")
    expect(scoreboard.get_by_text(f"+{seconds}s", exact=True)).to_be_visible(
        timeout=5000
    )
...
def banner_not_blocking(spa_page):
    expect(only_searchbox(spa_page)).to_be_enabled(timeout=5000)
...
@then('the host sees an "End Game" button')
