...
All tests are E2E tests. Application servers are started externally before running tests. Frontend tests may manage their own mock WebSocket server fixtures. Real-time waits (`time.sleep`, `wait_for_timeout`) are the correct approach for time-dependent behavior — test-only time manipulation is prohibited.
...
`pytest.skip` is acceptable only when a step can only be verified through a test-only mechanism. The skip reason must state why the step cannot be tested, not what mechanism would be needed.
...
Tests do not start or manage application servers. Start them externally before running tests.
