...
def game_with_n_no_handicap(make_browser_player, ctx, count):
    setup_room_with_players(make_browser_player, ctx, count - 1)
    _label_host_as_last_player(ctx, count)
    _start_game_with_play(ctx)


@when("player A submits the correct answer")
def player_a_correct(ctx):
    submitted = submit_correct_answer(ctx, ctx.named_players["A"])
...
def player_b_correct_after_a(ctx):
    submitted = submit_correct_answer(ctx, ctx.named_players["B"])
...
def _label_host_as_last_player(ctx, count: int):
    """Feature-level player counts include the host as one player."""
    assert ctx.host is not None
    label = chr(ord("A") + count - 1)
    ctx.named_players.setdefault(label, ctx.host)
...
def player_a_wrong_answer(ctx):
    ...
    submitted = submit_wrong_answer(ctx, player)
    assert submitted, "player A's wrong submission did not match any suggestion"
...
def b_and_c_can_answer(ctx):
    for label in ["B", "C"]:
        ...
        player = ctx.named_players[label]
        page = player.page
...
def both_submit_simultaneously(ctx):
    # Prepare both pages up to the final click, then fire the two submitting
    # clicks back-to-back on the main thread. Playwright's sync API forbids
    # driving pages from worker threads, so the clicks cannot be truly parallel;
    # they land within milliseconds, and the handicap (seconds) dominates that
    # gap, so the handicap—not arrival order—decides the scoring order.
    option_a = prepare_correct_answer(ctx, ctx.named_players["A"])
    option_b = prepare_correct_answer(ctx, ctx.named_players["B"])
    option_a.click()
    option_b.click()
    ctx.host.page.wait_for_timeout(2000)
...
def b_scores_before_a(ctx):
    ...
    assert int(b_pos) < int(a_pos), (
        f"Expected player B (pos {b_pos}) to score before player A (pos {a_pos})"
    )
...
def n_players_submit_correct(ctx, count):
    ...
    for i in range(count):
        submitted = submit_correct_answer(ctx, ctx.players[i])
...
def player_a_has_points(ctx):
    ...
    submitted = submit_correct_answer(ctx, player)
