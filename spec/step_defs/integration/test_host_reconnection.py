...
# room.feature specifies a 5-minute deletion grace; keep a 60-second margin
# so the polling assertion has room for scheduling and network jitter.
_ROOM_DELETION_POLL_TIMEOUT_SECONDS = 360
...
def host_can_control(ctx):
    ...
    expect(host_page.get_by_role("button", name="Play")).to_be_visible(timeout=5000)
...
def grace_period_expires(ctx, make_player_client):
    ...
    deleted = poll_until_room_deleted(
        lambda code: client.check_room(code),
        ctx.room_code,
        _ROOM_DELETION_POLL_TIMEOUT_SECONDS,
    )
    assert deleted, (
        f"Room {ctx.room_code} was not deleted within "
        f"{_ROOM_DELETION_POLL_TIMEOUT_SECONDS}s; "
...
def _disconnect_first_nonhost(ctx):
    ...
    submit_correct_answer(ctx, player)
...
def game_finished_with_players(make_browser_player, ctx, count):
    ...
    submit_correct_answer(ctx, ctx.players[0])
...
def another_player_in_room(make_browser_player, ctx):
    ...
    expect(player.page.get_by_text(re.compile(r"Players\s*\(1\)"))).to_be_visible(
        timeout=10000
    )
