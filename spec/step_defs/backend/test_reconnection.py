...
@then("game:player-state is sent to the host")
def player_state_sent_to_host(ctx):
    event = ctx.host.expect_event("game:player-state")
    assert "scored" in event["payload"]
    assert "wrongAnswerCount" in event["payload"]


@then("game:restore-reveal is sent to the host")
def restore_reveal_sent_to_host(ctx):
    event = ctx.host.expect_event("game:restore-reveal")
