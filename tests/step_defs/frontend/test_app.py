...
def session_api_fails(page, frontend_url, mock_ws_server):
    ...
    page.add_init_script(f"""
        window.__TEST_WS_URL__ = "{mock_ws_server.url}";
    """)
...
def text_displayed_as_entire_page(spa_page, text):
    expect(spa_page.get_by_role("main").filter(has_text=text)).to_be_visible(
        timeout=10000
    )
...
@given("the initial WebSocket connection fails")
def initial_ws_fails(mock_ws_server):
    mock_ws_server.stop()


@when("the app attempts the initial WebSocket connection")
def app_attempts_initial_ws_connection(spa_page, frontend_url, mock_ws_server):
    mock_ws_server.stop()
    spa_page.goto(frontend_url)
