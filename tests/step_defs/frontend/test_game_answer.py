...
def _submit_first_suggestion(spa_page, query: str):
    spa_page.get_by_role("searchbox").first.fill(query)
    spa_page.wait_for_timeout(500)
    spa_page.get_by_role("option").first.click()
...
def suggestion_details(spa_page):
    first = spa_page.get_by_role("option").first
    expect(first).to_contain_text("Song 1")
    expect(first).to_contain_text("Artist 1")
...
def no_suggestions(spa_page):
    spa_page.wait_for_timeout(500)
    expect(spa_page.get_by_role("option")).to_have_count(0)


@when("the player types a query that matches no song titles")
def type_no_match(spa_page):
    spa_page.get_by_role("searchbox").first.fill("zzzznonexistent")
...
def all_answer_ui_reset(spa_page):
    expect(spa_page.get_by_role("searchbox").first).to_have_value("")
    expect(spa_page.get_by_role("option")).to_have_count(0)
...
def try_submit(spa_page):
    ...
    search_input = spa_page.get_by_role("searchbox").first
...
def submit_answer(spa_page):
    _submit_first_suggestion(spa_page, "Song")


@then("a yellow panel shows the submitted song title")
def yellow_panel_shows_title(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="Song 1").first
...
def countdown_displayed(spa_page, seconds):
    ...
    countdown = spa_page.get_by_role("timer")
    expect(countdown.first).to_contain_text(str(seconds), timeout=5000)
    initial = countdown.first.inner_text()
    spa_page.wait_for_timeout(300)
    later = countdown.first.inner_text()
...
def progress_bar(spa_page):
    bar = spa_page.get_by_role("progressbar").first
...
@then(parsers.parse('the wrong answer title is displayed in red as "Wrong: {{title}}"'))
def wrong_answer_displayed(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="Wrong:").first
    ).to_be_visible(timeout=5000)
...
def search_disabled(spa_page):
    expect(spa_page.get_by_role("searchbox").first).to_be_disabled(timeout=5000)
...
@then(parsers.parse('a red panel shows "Locked out for {{N}}s" updating every 100ms'))
def lockout_panel(spa_page):
    panel = spa_page.get_by_role("status").filter(has_text="Locked out for").first
...
@then(parsers.parse('"{text}" is displayed in gray'))
def text_in_gray(spa_page, text):
    expect(spa_page.get_by_text(text, exact=False).first).to_be_visible(timeout=5000)
...
@then(parsers.parse('a blue panel shows "You scored {points:d} points!"'))
def blue_panel_scored(spa_page, points):
    expect(
        spa_page.get_by_role("status")
        .filter(has_text=f"You scored {points} points!")
        .first
    ).to_be_visible(timeout=5000)
...
def player_in_reveal(spa_page):
    reveal_panel = spa_page.get_by_role("region", name="Reveal").first
...
@then(
    parsers.parse(
        'a green panel shows "{{nickname}} scored {{points}}pt(s)! ({{rank}})"'
    )
)
@then('a green panel shows "{nickname} scored 4pt(s)! (1st)"')
def green_panel(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="scored").first
    ).to_be_visible(timeout=5000)
...
def ordinals_follow(spa_page):
    ...
    expect(spa_page.get_by_role("status").filter(has_text="1st").first).to_be_visible(
        timeout=5000
    )
...
def search_focused(spa_page):
    expect(spa_page.get_by_role("searchbox").first).to_be_focused(timeout=5000)
...
def scored_panel(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="You scored 4 points!").first
    ).to_be_visible(timeout=5000)
...
def lockout_countdown(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="Locked out for").first
    ).to_be_visible(timeout=5000)
...
def attempts_restored(spa_page):
    expect(
        spa_page.get_by_text("1 / 3 attempts left", exact=False).first
    ).to_be_visible(timeout=5000)
...
def handicap_countdown_remaining(spa_page):
    expect(spa_page.get_by_role("timer").filter(has_text="Song 1").first).to_be_visible(
        timeout=5000
    )
