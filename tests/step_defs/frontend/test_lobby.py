...
def _enter_lobby_as_host(spa_page, mock_ws_server, frontend_url, **kwargs):
    setup_scenario(
        spa_page,
        mock_ws_server,
        frontend_url,
...
def player_list_count(spa_page, count):
    players_section = spa_page.get_by_role("region", name="Players").first
...
def host_badge_visible(spa_page):
    host_entry = (
        spa_page.get_by_role("region", name="Players")
        .get_by_role("listitem")
        .filter(has_text="Host")
        .first
    )
    expect(host_entry).to_contain_text("Host", timeout=5000)
...
def handicap_badge_visible(spa_page, seconds):
    expect(spa_page.get_by_text(f"+{seconds}s").first).to_be_visible(timeout=5000)
...
def no_handicap_badge(spa_page):
    ...
    player_entry = (
        spa_page.get_by_role("region", name="Players").get_by_role("listitem").first
    )
...
def section_visible(spa_page, section):
    expect(spa_page.get_by_role("heading", name=section).first).to_be_visible(
        timeout=5000
    )
...
def search_input_visible(spa_page):
    expect(spa_page.get_by_role("searchbox").first).to_be_visible(timeout=5000)
...
def input_displayed(spa_page, text):
    expect(spa_page.get_by_role("searchbox", name=text).first).to_be_visible(
        timeout=5000
    )
...
def switch_tabs(spa_page, mock_ws_server, frontend_url):
    ...
    spa_page.get_by_role("searchbox").first.fill("Test query")
...
def tabs_retain_state(spa_page):
    spa_page.get_by_role("tab", name="Public").click()
    expect(spa_page.get_by_role("searchbox").first).to_have_value("Test query")
...
def playlist_loaded_directly(spa_page):
    ...
    expect(
        ...
        .or_(spa_page.get_by_role("heading", name="Songs").locator(":scope"))
        .first
...
def type_keyword(spa_page):
    spa_page.get_by_role("searchbox").first.fill("rock")


@then("matching playlists are displayed")
def matching_playlists(spa_page):
    expect(
        spa_page.get_by_role("listitem").filter(has_text="Playlist").first
    ).to_be_visible(timeout=5000)
...
def songs_replaced(spa_page):
    expect(
        spa_page.get_by_role("heading", name="Songs", exact=False).first
    ).to_be_visible(timeout=5000)
...
def loading_songs_displayed(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="Loading songs...").first
    ).to_be_visible(timeout=5000)
...
def loading_library_displayed(spa_page):
    expect(
        spa_page.get_by_role("status")
        .filter(has_text="Loading library playlists...")
        .first
    ).to_be_visible(timeout=5000)
...
def song_details(spa_page):
    songs_region = spa_page.get_by_role("region", name="Songs").first
    first_song = songs_region.get_by_role("listitem").first
...
def playback_help_text(spa_page):
    expect(spa_page.get_by_text("play for 1s", exact=False).first).to_be_visible(
        timeout=5000
    )
...
def rank_help_text(spa_page):
    expect(spa_page.get_by_text("1st gets 4pt", exact=False).first).to_be_visible(
        timeout=5000
    )
...
def settings_read_only(spa_page):
    expect(spa_page.get_by_text("Playback durations", exact=False).first).to_be_visible(
        timeout=5000
    )
