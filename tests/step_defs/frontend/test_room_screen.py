...
def ws_not_established(spa_page, frontend_state):
    # Point the SPA at a closed port so the connect attempt stalls; the
    # backoff retry covers the duration of the test.
    spa_page.add_init_script("""
        window.__TEST_WS_URL__ = "ws://localhost:59999";
    """)
...
def connecting_displayed(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="Connecting...").first
...
def joining_displayed(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="Joining room...").first
    ).to_be_visible(timeout=5000)
...
def message_as_toast(spa_page):
    expect(
        spa_page.get_by_role("alert").filter(has_text="Something went wrong").first
    ).to_be_visible(timeout=5000)
...
def ws_connection_lost(mock_ws_server):
    conn = mock_ws_server.latest_connection
...
def reconnecting_displayed(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="Reconnecting...").first
    ).to_be_visible(timeout=5000)
...
def all_retries_fail(spa_page, text):
    expect(spa_page.get_by_text(text, exact=False).first).to_be_visible(timeout=40000)
...
def message_toast(spa_page):
    expect(
        spa_page.get_by_role("alert").filter(has_text="Room has been closed").first
    ).to_be_visible(timeout=5000)
...
def screen_determined_by_state(spa_page, mock_ws_server):
    ...
    expect(spa_page.get_by_text("Round", exact=False).first).to_be_visible(timeout=5000)
...
def settings_ui_updated(spa_page):
    expect(spa_page.get_by_text("Updated Player").first).to_be_visible(timeout=5000)
...
def state_ui_updated(spa_page):
    expect(spa_page.get_by_text("Round 2", exact=False).first).to_be_visible(
        timeout=5000
    )
...
def no_auto_reconnect(spa_page):
    expect(
        spa_page.get_by_role("status").filter(has_text="Reconnecting...")
    ).to_have_count(0)
