...
def players_sorted(spa_page):
    leaderboard = spa_page.get_by_role("region", name="Final Scores").first
...
def player_shows_rank_name_score(spa_page):
    first_entry = (
        spa_page.get_by_role("region", name="Final Scores")
        .get_by_role("listitem")
        .first
    )
    expect(first_entry).to_contain_text("Player 2", timeout=5000)
    expect(first_entry).to_contain_text("12")
...
def same_rank(spa_page):
    leaderboard = spa_page.get_by_role("region", name="Final Scores").first
    expect(leaderboard.get_by_text("#1", exact=False)).to_have_count(2)


@then("the order of tied players is not defined")
def tied_order(spa_page):
    leaderboard = spa_page.get_by_role("region", name="Final Scores").first
    expect(leaderboard.get_by_text("Host")).to_be_visible(timeout=5000)
    expect(leaderboard.get_by_text("Player 2")).to_be_visible(timeout=5000)


@then("the first place player's card has a yellow background and border")
def first_place_highlight(spa_page):
    leaderboard = spa_page.get_by_role("region", name="Final Scores").first
    first_card = leaderboard.get_by_role("listitem").first
    expect(first_card).to_be_visible(timeout=5000)
    styles = first_card.evaluate(
        """(el) => {
    ...
    # Tailwind's yellow-* family resolves to colors where R and G are
    # both high and B is noticeably lower (yellow/amber hue).
    def is_yellow(color: str) -> bool:
        parts = color.removeprefix("rgb(").removeprefix("rgba(").rstrip(")").split(",")
        if len(parts) < 3:
            return False
        r, g, b = (int(p.strip()) for p in parts[:3])
        return r > 180 and g > 150 and b < r - 30 and b < g - 20

    assert is_yellow(styles["bg"]) or is_yellow(styles["border"]), (
        f"Expected first place card to be highlighted yellow, got "
        f"bg={styles['bg']!r} border={styles['border']!r}"
    )
...
def navigated_to_lobby(spa_page):
    expect(
        spa_page.get_by_role("heading", name="Players", exact=False).first
...
def disconnected_separate(spa_page):
    active = spa_page.get_by_role("region", name="Final Scores").first
